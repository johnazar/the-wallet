<template>
  <div>
    <div class="flex items-center justify-between mb-3">
      ...
      <button
        class="px-2 py-1 bg-sky-500 text-white rounded"
    ...
    <div v-if="showNew" class="mb-4 bg-white p-3 rounded shadow">
      ...
      <div class="flex gap-2">
        <button class="px-3 py-2 bg-green-500 text-white rounded" @click="add">
          Add
        </button>
        <button class="px-3 py-2 bg-gray-200 rounded" @click="cancel">
          Cancel
        </button>
      </div>
    </div>
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <div class="text-sm font-medium">
            {{ balances[a.id] ? formatMoney(balances[a.id]) : "-" }}
          </div>
          <button class="px-2 py-1 text-sm border rounded" @click="edit(a)">
            Edit
          </button>
          <button
...
async function remove(a: any) {
  ...
  if (count > 0) {
    if (
      !confirm(
        `Account has ${count} transactions. Delete account and its transactions?`,
      )
    )
      return;
...
function formatMoney(cents: number) {
  return centsToCurrency(
    cents,
    auth.users.find((u) => u.id === auth.currentUserId)?.currency ?? "USD",
  );
}
</script>
